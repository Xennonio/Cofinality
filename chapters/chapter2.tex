...
To facilitate notation, we will say that a cardinal $\aleph_\alpha$ is a \lasereye{successor cardinal} if $\alpha$ is a successor ordinal, and we will say that it's a \lasereye{limit cardinal} if $\alpha$ is a limit ordinal.
...
\begin{lemma}{}{successor_is_regular}
    Every successor cardinal $\aleph_{\alpha+1}$ is a regular cardinal.
\end{lemma}

\begin{proof}
    Assume by contradiction that $\aleph_{\alpha+1}$ is singular, by \cref{lem:singular_characterization}, it can be written as a sum of a smaller number of smaller cardinals:
...
\begin{corollary}{}{next_limit_singular}
...
\section{Uncountable Limit Regular Cardinals}\label{sec:weakly_inaccessible}
...
i.e., $\aleph_\lambda$ is the limit of an increasing sequence of length $\alpha$, in particular, we could change such sequence to another one of length $\cf{\lambda}$. It's easy to see how this imply the next result
\begin{lemma}{}{cof_aleph}
    If $\lambda$ is a limit ordinal, then $\cf{\aleph_\lambda}=\cf{\lambda}$
\end{lemma}

Since in our case $\aleph_\lambda$ is also a regular cardinal, then $\aleph_\lambda=\cf{\aleph_\lambda}=\cf{\lambda}\leq\lambda$, which imply
\begin{center}
    \begin{tcolorbox}[hbox]
    $\aleph_\lambda=\lambda$
    \end{tcolorbox}
\end{center}
Such cardinals are called \lasereye{aleph fixed points}, since, well, they're a fixed point of the aleph operation.
...
\begin{lemma}{}{aleph_fixed_points}
...
\begin{proof}
    ...
    Then $\alpha=\aleph_\alpha$ is a fixed aleph point which is also a limit of an increasing sequence of length $\omega$, i.e., $\cf{\alpha}=\omega<\alpha$, then it's singular.
\end{proof}

Another smallness property of the first aleph fixed point is that it has countable cofinality. Then, how far an uncountable limit regular cardinal should be in the ``line'' of aleph fixed points? Well, let $F$ be the class function enumerating all aleph fixed point, then, since $F$ is a normal function, $\kappa$ should also be a $F$-fixed point! I.e., it's as far in the line as its own size.

And we can keep going on this, if we let $G$ be an enumeration of the $F$ fixed point, then $G(\kappa)=\kappa$ too, since $G$ would be normal, and so on. This shows for itself how big such cardinals are.

The next natural step is then ask if we can even achieve an example of an uncountable regular limit cardinal. Well, surprisingly (or maybe not), such cardinals, named \lasereye{weakly inaccessible cardinals}, are so big that ZFC can't prove they exist! Inaccessible cardinals are some of the first cardinals in a larger class of properties named Large Cardinals Axioms.

This facts captures even better how big such cardinals are. In some sense, we can't even define it based on definable normal functions in ZFC, so almost everything ``really big'' you could try to create in the language of ZFC would not reach a weakly inaccessible.

Unfortunatelly, the proof ZFC can't prove its existence is not that easy, since we need to use a consistent result, namely, that GCH is consistent with ZFC\footnote{You can find a proof of it in my \href{https://github.com/Xennonio/Consistency-Results}{Consistency Results notes}, in Section 3. The Constructible Universe $L$.}. However, we can prove it for a somewhat similar cardinal, named strongly limit cardinals, you can see how to prove it in the \todo{add hyperlink}Large Cardinals notes, and then how to use it to show ZFC also can't prove weakly inaccessible exist.

\section{Rambling on Cofinality}\label{sec:ramb_cof}
...
Now, imagine we have some property about ordinals smaller than $\aleph_\omega$, and such property is somewhat inductive. Then the process of extending this property to $\aleph_\omega$ itself would require only $\omega$ many steps, since $\cf{\aleph_\omega}=\omega$, whereas, for example, doing the same for $\aleph_1$ would require uncountable many steps.
...
\begin{lemma}{}{cf_alpha_is_regular}
    If a limit ordinal $\alpha$ isn't a cardinal, then $\cf{\alpha}<\alpha$. In particular, for all limit ordinal $\alpha$, we have $\cf{\alpha}=\alpha$ if, and only if, $\alpha$ is a regular cardinal.
\end{lemma}
...
\begin{proof}
    ...
    Now, even if such sequence was not increasing, we could find, using transfinite induction, an increasing subsequence with limit $\alpha$. And, since the length of the subsequence is at most $\kappa<\alpha$, then $\cf{\alpha}<\alpha$.
\end{proof}

\begin{lemma}{}{cf_idempotency}
    For all limit ordinal $\alpha$, we have
    $$\cf{\cf{\alpha}}=\cf{\alpha}$$
\end{lemma}

\begin{proof}
    Let $\vartheta=\cf{\alpha}$. Clearly $\vartheta$ is a limit ordinal and we already know that $\cf{\vartheta}\leq\vartheta$. Now, assume by a contradiction that $\gamma=\cf{\vartheta}<\vartheta$, then there is an increasing sequence $\langle\nu_\xi:\xi<\gamma\rangle$ with $\lim_{\xi\to\gamma}\nu_\xi=\vartheta$. But since $\vartheta=\cf{\alpha}$, there is also a $\langle\alpha_{\nu_\xi}:\xi<\gamma\rangle$ of length $\gamma<\vartheta$ with limit $\alpha$, contradicting the minimality of $\vartheta$.
\end{proof}

\begin{corollary}{}{cf_alpha_is_regular}
    For all limit ordinal $\alpha$, $\cf{\alpha}$ is a regular cardinal.
\end{corollary}