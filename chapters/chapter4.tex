...
In order to prove Silver's Theorem, we will need a really powerful concept called stationary sets. Besides Silver's Theorem, we will also see stationary sets (more specifically, Fodor's Lemma) proves a lot of other important results.
...
\begin{definition}{Club}{club}
    A set $C\subseteq\kappa$ is called a \lasereye{closed unbounded set in $\kappa$} (or, more shortly, a \lasereye{club}), if
    \begin{itemize}
...
The previous lemma guarantees us the set of all clubs generates a filter, called \lasereye{the club filter} on $\kappa$
...
\begin{definition}{Stationary Set}{stationary}
    A set $S\subseteq\kappa$ is \lasereye{stationary} if for every club $C$, we have $S\cap C\neq\emptyset$.
\end{definition}
...
\begin{definition}{Regressive Function}{regressive}
    A function $f$ with domain $S\subseteq\kappa$ is \lasereye{regressive} if $f(\alpha)<\alpha$ for all $\alpha\neq0$.
\end{definition}
...
\begin{lemma}{}{diagonal_intersection}
    ...
    called the \lasereye{Diagonal Intersection} of the $C_\xi$, is also a club.
\end{lemma}
...
\begin{definition}{$\Delta$-system}{delta_system}
    A family of sets $\mc{A}$ forms a \lasereye{$\Delta$-system with root $R$} if, and only if, $X\cap Y=R$, whenever $X,Y\in\mc{A}$, with $X\neq Y$.
\end{definition}
...
The original theorem appeared in 1974 at \cite{bib:silver}, in an article called ``On the singular cardinals problem'', and stated that GCH couldn't fail for the first time in a singular cardinal of uncountable cofinality. This result was inspired by the work of Magidor, who at the time was interested in proving that certain types of ultrafilters, called regular ultrafilters, did not exist in $\omega_1$. Instead, he proved that if they did exist, Silver's theorem holds for $\lambda=\omega_1$!\footnote{Not a factorial joke.}

So, roughly, based on Magidor's results, instead of trying to construct directly such object, Silver extends the universe through forcing! He adds to the ground model $M$ a bijection between $2^{\cf{\kappa}}$ and $\aleph_0$ to make it countable, which simplify the combinatorics a lot. He then construct an ultraproduct of the ground model in which $\mc{P}(\kappa)$ is small, allowing him to ``reflect'' such result and conclude $\mc{P}(\kappa)$ is also small in the ground model $M$. A complete proof can be found in \cite{bib:gallart}.

Knowing that, many people simplified his proof in such a way that nowadays we can prove it in introductory set theory books. Despite this, much of the motivation behind what is used remains in Silver's original idea, and then it may seem quite strange.
...
\begin{definition}{Strong Limit Cardinal}{strong_limit}
    A cardinal $\kappa$ is called a \lasereye{strong limit cardinal}\footnote{Yeah, this is the same strong limit we cited in the end of \cref{sec:weakly_inaccessible}} if, for any $\lambda<\kappa$, we have $2^\lambda<\kappa$.
\end{definition}
...
\lasereye{Throughout this section, we will fix a $\lambda$ as in Silver's Theorem, and let $\kappa=\cf{\lambda}<\lambda$}. Then, by the previous Lemma, we can conclude the following, which we will use extensively from now on
...
\begin{definition}{Almost Disjoint}{almost_disjoint}
    Two functions $f,g$ on $\kappa$ are \lasereye{almost disjoint} if there is some $\alpha<\kappa$ such that $f(\beta)\neq g(\beta)$, for all $\beta\geq\alpha$.
\end{definition}
