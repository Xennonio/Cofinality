...
While addition and multiplication of cardinals are very simple, since
...
\end{lemma}
Note that this lemma restricts the possible values $2^{\aleph_\alpha}$ can assume, depending on its cofinality. As an example, which also reinforce the comments in \cref{sec:ramb_cof} and in the last footnote, notice that we can't have $2^{\aleph_0}=\aleph_\omega$, since then we would have $\aleph_0=\cf{\aleph_\omega}=\cf{2^{\aleph_0}}>\aleph_0$, a contradiction.

In fact, Robert Solovay published a result in a really famous one page paper named ``$2^{\aleph_0}$ can be anything it ought to be'' which basically says that the only thing we can prove about the continuum is that it has uncountable cofinality. Formally, the result says that for any cardinal $\kappa$ with uncountable cofinality, there is a model of ZFC where this is realized, i.e., where $2^{\aleph_0}=\kappa$.
...
\begin{proof}
    Let $\vartheta=\cf{2^{\aleph_\alpha}}$, then, there are $\kappa_\nu<2^{\aleph_\alpha}$, $\nu<\vartheta$ such that
    $$2^{\aleph_\alpha}=\sum_{\nu<\vartheta}\kappa_\nu$$
    ...
    a contradiction, so $\vartheta=\cf{2^{\aleph_\alpha}}>\aleph_\alpha$.
\end{proof}
...
Schematically, we obtain \cref{fig:singular_gch}\footnote{where the eventual linearity and denseness are poetic license.}.
...
\begin{proof}
    If $\beta\geq\alpha$, then $\aleph_\beta\geq\aleph_\alpha$ and $\aleph_\alpha^{\aleph_\beta}=2^{\aleph_\beta}=\aleph_{\beta+1}$. If $\aleph_\beta<\cf{\aleph_\alpha}$, then any $X\subseteq\omega_\alpha$ with $|X|=\aleph_\beta<\cf{\aleph_\alpha}<\aleph_\alpha$ is bounded in $\omega_\alpha$ and, therefore, by the same argument as in the previous theorem, we have $\aleph_\alpha^{\aleph_\beta}=\aleph_\alpha$. Let then $\cf{\aleph_\alpha}\leq\aleph_\beta\leq\aleph_\alpha$. On the one hand
    ...
    \end{equation}
    But on the other, by \cref{cor:cf_exponentiation}, $\cf{\aleph_\alpha^{\aleph_\beta}}>\aleph_\beta$ and, since $\aleph_\beta\geq\cf{\aleph_\alpha}$, then $\cf{\aleph_\alpha^{\aleph_\beta}}\neq\cf{\aleph_\alpha}$, so $\aleph_\alpha^{\aleph_\beta}\neq\aleph_\alpha$. With this and \cref{eq:aux_eq}, the only possible value for $\aleph_\alpha^{\aleph_\beta}$ is $\aleph_{\alpha+1}$.
\end{proof}
...
\begin{proof}
    If $\beta\geq\alpha+1$, then, since $\aleph_{\alpha+1}\leq\aleph_\beta<2^{\aleph_\beta}$, we have, by \cref{lem:card_exp_a_leq_b}
    ...
    Therefore, it's sufficient to show $\aleph_{\alpha+1}^{\aleph_\beta}\leq\aleph_\alpha^{\aleph_\beta}\cdot\aleph_{\alpha+1}$. Since $\omega_\beta<\omega_{\alpha+1}$ and $\omega_{\alpha+1}$ is regular, then for any $f:\omega_\beta\to\omega_{\alpha+1}$, $f$ is bounded, i.e., there is some $\gamma<\omega_{\alpha+1}$ such that $f(\xi)<\gamma$, for every $\xi<\omega_\beta$; in other words $f:\omega_\beta\to\gamma$. With this, we obtain
