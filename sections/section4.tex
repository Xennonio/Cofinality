...
The previous lemma guarantees us the set of all clubs generates a filter, called the club filter on $\kappa$:
$$F=\{X\subseteq\kappa:X\supseteq C\text{ for some club }C\}$$
...
\begin{definition}{}{}
    A family of sets $\mc{A}$ forms a $\Delta$-system with root $R$ if, and only if, $X\cap Y=R$, whenever $X,Y\in\mc{A}$, with $X\neq Y$.
\end{definition}

We also say $\mc{B}$ forms a $\Delta$-system, if there is a set $R$ such that $\mc{B}$ is a $\Delta$-system with root $R$.

\begin{corollary}{$\Delta$-System Lemma}{}
...
\begin{proof}
    We may assume WLOG that $\mc{A}\subseteq[\kappa]^{<\omega}$. Since every set in $\mc{A}$ is finite, and we have $\kappa$-many of those, by Pigeonhole we can find $\kappa$ of them $\{A_\alpha:\alpha<\kappa\}$ of the same size $n\in\omega$.
...
Let's now prove the main theorem of this section. Roughly, it says that if GCH holds for all cardinals smaller than some singular with uncountable cofinality, then it also holds for it.

\begin{theorem}{Silver's Theorem}{}
    Let $\aleph_\lambda$ be a singular cardinal, with $\text{cf}(\lambda)>\omega$. If $2^{\aleph_\alpha}=\aleph_{\alpha+1}$, for every $\alpha<\lambda$, then
    $$2^{\aleph_\lambda}=\aleph_{\lambda+1}$$
\end{theorem}

Now, if $\lambda$ itself is not singular, 

\todo{different statement in the general case}

In order to prove it, we will first prove a series of lemmas needed to show Silver's result.

\begin{definition}{Strong Limit Cardinal}{}
    A cardinal $\kappa$ is called a \textcolor{laser_eyes!80!black}{strong limit cardinal} if, for any $\lambda<\kappa$, we have $2^\lambda<\kappa$.
\end{definition}

It's easy to see that, under GCH, the concept of a limit cardinal matches with the one of a strong limit\footnote{yeah, this is the same strong limit we cited in the end of section \ref{weaky_inacces}} cardinal. Why we should care? Look again to Silver's Theorem hypothesis, we have some kind of partial GCH for all cardinal smaller than $\aleph_\lambda$. In fact, this imply that $\aleph_\alpha$ is a strong limit cardinal for every $\alpha<\lambda$. In particular, the next lemma holds.
...
\begin{proof}
    ...
    where in the inequality we used Lemma \ref{lemma_for_a<=b}.
\end{proof}

Throughout this section, we will fix a $\lambda$ as in Silver's Theorem, and let $\kappa=\text{cof}(\lambda)$. Then, by the previous Lemma, we can conclude that 

\todo{todo}

\begin{lemma}{}{}
    Let $\{A_\alpha:\alpha<\kappa\}$ be a family such that $\lvert A_\alpha\rvert\leq\aleph_\alpha$ for every $\alpha<\kappa$. If $F$ is a family of almost disjoint choice functions for the family, then $\lvert F\rvert\leq\aleph_\kappa$.
\end{lemma}

\begin{proof}
    We can assume WLOG that $A_\alpha\subseteq\omega_\alpha$. Let $S_0$ be the set of all limit ordinals between $0$ and $\kappa$. Given $f\in F$ and $\alpha\in S_0$, let $f^*(\alpha)$ be the least $\beta$ such that $f(\alpha)<\omega_\beta$.

    Since $f(\alpha)\in A_\alpha\subseteq\omega_\alpha$, we have $f(\alpha)<\omega_\alpha$ and, since $\alpha$ is a limit ordinal, then $f(\alpha)<\omega_\beta$, for some $\beta<\alpha$. This implies that $f^*(\alpha)<\alpha$, i.e., it's a regressive function. Since $S_0$ is stationary, by Fodor's Lemma there is some stationary $S\subseteq S_0$ such that $f^*\restriction_S$ is constant, let's say in $\beta$. Then $f\restriction_S$ is a function from $S\subseteq\kappa$ to $\omega_\beta<\omega_\kappa$. Note that both $S$ and $\beta$ depends on $f$.
    ...
    $$\lvert F\rvert\leq\sum_{S\subseteq\kappa}\sum_{\beta<\kappa}\omega_\beta^{\lvert S\rvert}\leq 2^\kappa\sum_{\beta<\kappa}\omega_\beta^\kappa$$
\end{proof}
