...
\begin{equation}\label{2_exp_prop}
...
The idea behind the proof is: If $\kappa_i<\lambda_i$, then every function $f_i:\kappa_i\to\lambda_i$ isn't surjective, therefore, we can find an element $b_i\in\lambda_i\backslash\text{Im}(f_i)$, and then we obtain, for each function $f:\bigcup_{i\in I}A_i\to\prod_{i\in I}B_i$, the element $b=\langle b_i:i\in I\rangle$ which is not in the image of $f$.

\begin{proof}
    Let $\langle A_i:i\in I\rangle$ and $\langle B_i:i\in I\rangle$ be such that $|A_i|=\kappa_i$ and $|B_i|=\lambda_i$, with $A_i$ mutually disjoint, $\forall i\in I$. Since $|A_i|<|B_i|$, then, given $f_i:A_i\to B_i$ injective, we know they can't be injective. So, define
...
\end{lemma}
Note that this lemma restricts the possible values $2^{\aleph_\alpha}$ can assume, depending on its cofinality. As an example, which also reinforce the comments in section \ref{ramb_cof} and in the last footnote, notice that we can't have $2^{\aleph_0}=\aleph_\omega$, since then we would have $\aleph_0=\cf\rp{\aleph_\omega}=\cf\rp{2^{\aleph_0}}>\aleph_0$, a contradiction.
...
Nowadays, such result is a direct consequence of a more general one known as Easton's Theorem, that says basically the same thing, but for any regular cardinal, i.e., the only constraints on permissible values for $2^\kappa$ when $\kappa$ is a regular cardinal are (\ref{2_exp_prop}) and the previous lemma.
...
\begin{proof}
    ...
    a contradiction, so $\vartheta=\text{cf}\rp{2^{\aleph_\alpha}}>\aleph_\alpha$.
\end{proof}
...
\begin{lemma}{}{}\label{lemma_for_a<=b}
...
\begin{proof}
    Since $|\omega_\alpha\times\omega_\beta|=\aleph_\alpha\cdot\aleph_\beta=\aleph_\alpha=|\omega_\alpha|$, then $\left|[\omega_\alpha\times\omega_\beta]^{\aleph_\beta}\right|=\left|[\omega_\alpha]^{\aleph_\beta}\right|$. Now, every function $f:\omega_\beta\to\omega_\alpha$ is in $[\omega_\alpha\times\omega_\beta]^{\aleph_\beta}$, therefore $\omega_\alpha^{\omega_\beta}\subseteq[\omega_\alpha\times\omega_\beta]^{\aleph_\beta}$ and so
    ...
    For the reverse inequality, if $X\in[\omega_\alpha]^{\aleph_\beta}$, then there is some $f$ in $\omega_b$ such that $\text{Im}(f)=X$. With this, given a function $f_X$ for each $X\in[\omega_\alpha]^{\aleph_\beta}$, let $F(X)=f_X$. If $X\neq Y$, then $\text{Im}(f_X)=X$ and $\text{Im}(f_Y)=Y$, so $F(X)=f_X\neq f_Y=F(y)$, i.e., $F$ is injective, then
...
\begin{proof}
    If $\alpha\leq\beta$, by Lemma \ref{lemma_for_a<=b} $\aleph_\alpha^{\aleph_\beta}=2^{\aleph_\beta}=\aleph_{\beta+1}$. Let then $\alpha>\beta$. If we let $S=[\omega_\alpha]^{\aleph_\beta}$, by Lemma \ref{lemma_for_a>=b} we have $|S|=\aleph_\alpha^{\aleph_\beta}$. In section \ref{ramb_cof}, we saw $\text{cf}(\kappa)$ can be interpreted as the least size of an unbounded set in $\kappa$. In particular, since $\cf(\aleph_\alpha)=\aleph_\alpha$ for regular cardinals, we know every $X\subseteq\omega_\alpha$, with $|X|<\aleph_\alpha$, is limited. Let then
...
\draw    (100,121) -- (554,121) ;
\draw [shift={(556,121)}, rotate = 180] [color={rgb, 255:red, 0; green, 0; blue, 0 }  ][line width=0.75]    (10.93,-3.29) .. controls (6.95,-1.4) and (3.31,-0.3) .. (0,0) .. controls (3.31,0.3) and (6.95,1.4) .. (10.93,3.29)   ;
\draw    (230.5,112.75) -- (230.5,129) ;
\draw    (400.5,112.25) -- (400.5,128.5) ;
\draw (205.5,133.9) node [anchor=north west][inner sep=0.75pt]    {$cf( \aleph _{\alpha })$};
\draw (390,134.9) node [anchor=north west][inner sep=0.75pt]    {$\aleph _{\alpha }$};
\draw (275.5,41.4) node [anchor=north west][inner sep=0.75pt]    {$f( \aleph _{\beta }) =\aleph _{\alpha }^{\aleph _{\beta }}$};
\draw (151,93.9) node [anchor=north west][inner sep=0.75pt]    {$\aleph _{\alpha }$};
\draw (301,93.9) node [anchor=north west][inner sep=0.75pt]    {$\aleph _{\alpha +1}$};
\draw (458,93.9) node [anchor=north west][inner sep=0.75pt]    {$\aleph _{\beta +1}$};
\end{tikzpicture}
\end{center}

\begin{proof}
    If $\beta\geq\alpha$, then $\aleph_\beta\geq\aleph_\alpha$ and $\aleph_\alpha^{\aleph_\beta}=2^{\aleph_\beta}=\aleph_{\beta+1}$. If $\aleph_\beta<\cf\rp{\aleph_\alpha}$, then any $X\subseteq\omega_\alpha$ with $|X|=\aleph_\beta<\cf\rp{\aleph_\alpha}<\aleph_\alpha$ is bounded in $\omega_\alpha$ and, therefore, by the same argument as in the previous theorem, we have $\aleph_\alpha^{\aleph_\beta}=\aleph_\alpha$. Let then $\cf\rp{\aleph_\alpha}\leq\aleph_\beta\leq\aleph_\alpha$. On the one hand
    \begin{equation}\tag{$\dagger$}
    \aleph_\alpha\leq\aleph_\alpha^{\aleph_\beta}\leq\aleph_\alpha^{\aleph_\alpha}=2^{\aleph_\alpha}=\aleph_{\alpha+1}
    \end{equation}
    But on the other, by Corollary \ref{cf(kl)>l}, $\cf\rp{\aleph_\alpha^{\aleph_\beta}}>\aleph_\beta$ and, since $\aleph_\beta\geq\cf\rp{\aleph_\alpha}$, then $\cf\rp{\aleph_\alpha^{\aleph_\beta}}\neq\cf\rp{\aleph_\alpha}$, so $\aleph_\alpha^{\aleph_\beta}\neq\aleph_\alpha$. With this and $(\dagger)$, the only possible value for $\aleph_\alpha^{\aleph_\beta}$ is $\aleph_{\alpha+1}$.
\end{proof}
...
\begin{theorem}{Hausdorff's Formula}{}
    \textbf{} For any ordinals $\alpha$ and $\beta$, we have
    $$\aleph_{\alpha+1}^{\aleph_\beta}=\aleph_\alpha^{\aleph_\beta}\cdot\aleph_{\alpha+1}$$
\end{theorem}

\begin{proof}
    If $\beta\geq\alpha+1$, then, since $\aleph_{\alpha+1}\leq\aleph_\beta\leq2^{\aleph_\beta}$, we have
